...
public class Inventory_Player : InventoryThing
{
    ...
    public bool CheckSlots(Items aItem)
    {
        ...
        foreach (Inventory_SlotThing aSlot in thisSlotList)
        {
            ...
            // Added this lines of code. Or it will be a bug.
            // Bug: while player's last inventory block has something inside. The system will keep think the inventory still has empty slot so it will keeps adding new items.
            else
            {
                if (thisSlotList.IndexOf(aSlot) == thisSlotList.Count - 1)
                {
